...
class EDPOPXShell(cmd2.Cmd):
    ...
    def show_record(self, record: Record) -> None:
        ...
        self.poutput(cmd2.ansi.style_success(record, bold=True))
    ...
    def _show_records(
        self, records: List[Optional[Record]], start: int, limit=math.inf
    ) -> int:
        ...
        for i in range(start, start + count):
            print("{:{digits}} - {}".format(i + 1, str(records[i]), digits=digits))
        return count
    ...
    def _query(self, readerclass: Type[Reader], query: str):
        ...
        try:
            if not self.exact:
                ...
                self.pfeedback("Performing exact query: {}".format(query))
            self.reader.fetch()
        except ReaderError as err:
            ...
            self.shown = 0
            return
        self.pfeedback("{} records found.".format(self.reader.number_of_results))
        self.shown += self._show_records(
            self.reader.records, self.shown, self.RECORDS_PER_PAGE
        )
