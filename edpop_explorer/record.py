...
class Record:
    ...
    def add_field(self, attribute_name: str, field: Field):
    ...
    def get_first_field(self, attribute_name: str) -> Optional[Field]:
        """Get a field based on its attribute name. If the field is repeated,
    ...
    def get_property_by_attribute(self, attribute: str) -> URIRef:
        """Get the RDF property IRI as a URIRef by Python attribute name."""
        return next((x[1] for x in self._fields if x[0] == attribute), None)

    def bind_all_fields(self) -> None:
        """Bind all fields of the record to their parent, to allow
        generation of the fields' IRIs. This method is called automatically
        before an RDF graph is generated by the ``to_graph()`` method."""
        for definiton in self._fields:
            attribute = definiton[0]
            ...
            for field in value:
                if isinstance(field, Field):
                    field.bind_parent_record(self, attribute)
                else:
                    raise RecordError("Record field is not a Field instance")
...
class BibliographicalRecord(Record):
    ...
    def __str__(self) -> str:
        if self.title:
            return str(self.get_first_field("title"))
        else:
...
class BiographicalRecord(Record):
    ...
    def __str__(self) -> str:
        if self.name:
            return str(self.get_first_field("name"))
        else:
            return super().__str__()
